...
jQuery(function ($) {
  ...
  $(window).on("scroll", function () {
    var target = ".header, .header__nav-item a, .header__logo-main, .header__logo-sub, .header__hamburger span";
    if ($(this).scrollTop() > 100) $(target).addClass("js-change-color");else $(target).removeClass("js-change-color");
  });
});
jQuery(function ($) {
  jQuery(function ($) {
...
jQuery(function ($) {
  ...
  function getFocusable($container) {
    return $container.find('a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]').filter(":visible");
  }
  ...
  // =====================================
  jQuery(window).on("scroll", function () {
    var targetSelector = ".header, .header__nav-item a, .header__logo-main, .header__logo-sub, .header__hamburger span";
    if (jQuery(this).scrollTop() > 100) {
  ...
  if (hasGSAP && hasScrollTrigger && document.querySelector(".js-fade-up-wrapper, .js-fade-up")) {
    // ペア用（画像＋テキスト）重複統合版
    document.querySelectorAll(".js-fade-up-wrapper").forEach(function (wrapper) {
      if (wrapper.dataset.fadeUpInitialized === "true") return;
      var imgFromWrapper = wrapper.querySelector(".js-fade-up-imgWrapper img");
      var imgPlain = wrapper.querySelector(".js-fade-up-img");
      var img = imgFromWrapper || imgPlain;
      var text = wrapper.querySelector(".js-fade-up-text");
      if (!img || !text) return;
    ...
    // 単独用
    document.querySelectorAll(".js-fade-up").forEach(function (el) {
  ...
  if (hasGSAP && hasScrollTrigger && document.querySelector(".js-colorChange")) {
    $(".js-colorChange").each(function () {
      ...
      $textElement.empty();
      var spans = [];
      $.each(originalText.split(""), function (i, _char) {
        var $span = $("<span>").text(_char);
...
$(window).on("load", function () {
  ...
  function splitTextToSpans(text) {
    return text.split("").map(function (_char2) {
      if (_char2 === " ") return '<span class="char is-space">&nbsp;</span>';
      if (_char2 === "&") return '<span class="char">&amp;</span>';
      return "<span class=\"char\">".concat(_char2, "</span>");
    }).join("");
  }
  ...
  $(".js-splitText").each(function () {
    ...
    var html = "\n      <span class=\"text-wrap\">\n        <span class=\"text-original\">".concat(originalText, "</span>\n        <div class=\"before\">").concat(spanHTML, "</div>\n        <div class=\"after\">").concat(spanHTML, "</div>\n      </span>\n    ");
    $target.html(html);
    var beforeSpans = $target.find(".before .char").toArray();
    var afterSpans = $target.find(".after .char").toArray();

    // 初期位置と非表示セット（チラつき防止）
    gsap.set(beforeSpans, {
      y: "0%"
    });
    gsap.set(afterSpans, {
      y: "100%",
    ...
    allBeforeSpans.push.apply(allBeforeSpans, _toConsumableArray(beforeSpans));
  ...
  if ($("body").hasClass("is-top")) {
    ...
    titles.forEach(function (selector) {
      var $el = $(selector);
      var chars = $el.text().split("").map(function (_char3) {
        return _char3 === " " ? "&nbsp;" : _char3;
      }).map(function (c) {
        return "<span>".concat(c, "</span>");
      }).join("");
      $el.html(chars);
    });
...
// MV パララックス固定
// =====================================
if (hasGSAP && hasScrollTrigger && document.querySelector(".mv") && document.querySelector(".mv__sticky-wrap")) {
...
// MV 背景拡大アニメーション
// =====================================
if (hasGSAP && hasScrollTrigger && document.querySelector(".mv") && document.querySelector(".mv-swiper")) {
  gsap.to(".mv-swiper", {
    scale: 1.15,
    ease: "none",
    scrollTrigger: {
...
// MV タイトルのフェードアウト
// =====================================
if (hasGSAP && hasScrollTrigger && document.querySelector(".mv") && document.querySelector(".mv__title-wrap")) {
